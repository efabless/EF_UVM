...
class wrapper_wb_monitor(UVMMonitor):
    def __init__(self, name="wrapper_wb_monitor", parent=None):
    ...
    async def recieve_transaction(self):
        while True:
            ...
            if self.vif.cyc_i.value.binstr == "1" and self.vif.stb_i.value.binstr == "1":
                break
        ...
        if write:
            ...
            try:
                ...
                uvm_error(self.tag, f"HRDATA is not an integer {self.vif.dat_o.value.binstr}")
                data = self.vif.dat_o.value.binstr
        return address, write, data


uvm_component_utils(wrapper_wb_monitor)
